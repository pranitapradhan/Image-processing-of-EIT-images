...
for i = 1:1:length(max_pks)
   if (max_pks(i)>1.5e04 && max_pks(i)< 2.0e04)
...
for i = 1:1:length(min_pks)
   if (min_pks(i)>-1.5e04 && min_pks(i)<-1.0e04)
...
max_meanframe=Maximas([2:14,16:23,25:31,33:39,41:end]);
min_meanframe=Minimas([1:13,15:22,24:30,32:38,40:end]);
...
for i = 1:1:length(max_pks_New)
   if (max_pks_New(i)>1.3e04 && max_pks_New(i)< 2.0e04)
...
for i = 1:1:length(min_pks_New)
   if (min_pks_New(i)<-0.8e04 && min_pks_New(i)> -1.4e04)
...
max_TV= Maximas_New_pks([2:14,16:23,25:31,33:39,41:end]);
min_TV= Minimas_pks_New([1:13,15:22,24:30,32:38,40:end]);
...
ratio_VT = Tidal_vol/Body_plyt(3,2);
ratio_ERV = ERV/Body_plyt(3,3);
ratio_IRV = IRV/Body_plyt(3,4);
ratio_FRC = FRC/Body_plyt(3,1);