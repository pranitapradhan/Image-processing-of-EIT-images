...
for i = 1:1:length(max_pks)
   if (max_pks(i)>1.97e04 && max_pks(i)< 2.5e04)
...
for i = 1:1:length(min_pks)
   if (min_pks(i)<-1.5e04 && min_pks(i)> -2e04)
...
max_meanframe=Maximas([1:8,10:17,19:25,27:33,35:end-1]);
min_meanframe=Minimas([1:8,10:17,19:25,27:33,35:end]);
...
for i = 1:1:length(max_pks_New)
   if (max_pks_New(i)>1.75e04 && max_pks_New(i)< 2.3e04)
...
for i = 1:1:length(min_pks_New)
   if (min_pks_New(i)<-1.286e04 && min_pks_New(i)> -1.5e04)
...
max_TV= Maximas_New_pks([1:8,10:17,19:25,27:33,35:end-1]);
min_TV= Minimas_pks_New([1:8,10:17,19:25,27:33,35:end]);
...
ratio_VT = Tidal_vol/Body_plyt(2,2);
ratio_ERV = ERV/Body_plyt(2,3);
ratio_IRV = IRV/Body_plyt(2,4);
ratio_FRC = FRC/Body_plyt(2,1);
