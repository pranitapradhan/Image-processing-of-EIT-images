...
for i = 1:1:length(max_pks)
   if (max_pks(i)>1.4e04 && max_pks(i)< 2.0e04)
    Maximas(end+1) = max_locs(i);
  end
end
Maximas =Maximas(1:end-1);
...
for i = 1:1:length(min_pks)
   if (min_pks(i)<-1.140e04 && min_pks(i)>-1.5e04)
...
max_meanframe=Maximas([1:9,11:17,19:25,27:32,34:end]);
min_meanframe=Minimas([1:6,8,10:11,14:20,23:29,32:37,40:end]);
...
for i = 1:1:length(max_pks_New)
   if (max_pks_New(i)>1.2e04 && max_pks_New(i)< 1.5e04)
...
for i = 1:1:length(min_pks_New)
   if (min_pks_New(i)<-0.8e04 && min_pks_New(i)> -1.5e04)
...
max_TV= Maximas_New_pks([1:9,11:17,19:25,27:32,34:end-1]);
min_TV= Minimas_pks_New([1:6,8,10:11,14:20,23:29,32:37,40:end-1]);
...
ratio_VT = Tidal_vol/Body_plyt(4,2);
ratio_ERV = ERV/Body_plyt(4,3);
ratio_IRV = IRV/Body_plyt(4,4);
ratio_FRC = FRC/Body_plyt(4,1);