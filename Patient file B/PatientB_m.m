...
for i = 1:1:length(max_pks)
   if (max_pks(i)>1.5e04 && max_pks(i)< 2.5e04)
...
for i = 1:1:length(min_pks)
   if (min_pks(i)<-0.9e04 && min_pks(i)> -1.5e04)
...
max_meanframe=Maximas([1:9,11:16,18:23,25:end]);
min_meanframe=Minimas([1:9,11:16,18:23,25:end]);
...
for i = 1:1:length(max_pks_New)
   if (max_pks_New(i)>1.2e04 && max_pks_New(i)< 2.0e04)
...
for i = 1:1:length(min_pks_New)
   if (min_pks_New(i)<-0.7e04 && min_pks_New(i)> -1.5e04)
...
max_TV= Maximas_New_pks([1:9,11:16,18:23,25:end]);
min_TV= Minimas_pks_New([1:8,10,12:17,19:24,26:end]);
...
ratio_VT = Tidal_vol/Body_plyt(5,2);
ratio_ERV = ERV/Body_plyt(5,3);
ratio_IRV = IRV/Body_plyt(5,4);
ratio_FRC = FRC/Body_plyt(5,1);