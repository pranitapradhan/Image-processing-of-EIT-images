...
%---- Finding Frame for Tidal Volume---%
 max_TV_locs=Maximas_New([1:9,11:16,18:23,25:end]);
 min_TV_locs=Minimas_New([1:8,10,12:17,19:24,26:end]);
...
FB4=F1(1:16,17:32);
 ...
 FB1_sum = sum(sum(FB1));
 rat1=FB1_sum/Body_plyt(5,2);
 
 %--- Tidal Volume for 2st half--%
 FB2_sum = sum(sum(FB2));
 rat2=FB2_sum/Body_plyt(5,2);
 
 %--- Tidal Volume for 3rd half--%
 FB3_sum = sum(sum(FB3));
 rat3=FB3_sum/Body_plyt(5,2);
 
 %--- Tidal Volume for 4th half--%
 FB4_sum = sum(sum(FB4));
 rat4=FB4_sum/Body_plyt(5,2);
