...
for i = 1:1:length(max_pks)
   if (max_pks(i)>1.2e04 && max_pks(i)< 1.7e04)
...
for i = 1:1:length(min_pks)
   if (min_pks(i)>-1.0e04 && min_pks(i)<-0.5e04)
...
max_meanframe=Maximas([1:9,11:16,18:22,24:29,31]);
min_meanframe=Minimas([1:9,12:17,19:23,25:30,32]);
...
for i = 1:1:length(max_pks_New)
   if (max_pks_New(i)>1.0e04 && max_pks_New(i)< 1.4e04)
...
for i = 1:1:length(min_pks_New)
   if (min_pks_New(i)<-0.50e04 && min_pks_New(i)> -1.0e04)
...
max_TV= Maximas_New_pks([1:9,11:16,18:22,24:29,31]);
min_TV= Minimas_pks_New([1:9,11:17,19:23,25:29,31]);
...
ratio_VT = Tidal_vol/Body_plyt(6,2);
ratio_ERV = ERV/Body_plyt(6,3);
ratio_IRV = IRV/Body_plyt(6,4);
ratio_FRC = FRC/Body_plyt(6,1);