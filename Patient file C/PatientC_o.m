...
for i = 1:1:length(max_pks)
   if (max_pks(i)>2.2e04 && max_pks(i)< 3.0e04)
...
for i = 1:1:length(min_pks)
   if (min_pks(i)<-1.40e04 && min_pks(i)>-2.1e04)
...
max_meanframe=Maximas([2:14,16:23,25:33,35:42,44:51,53:end-1]);
min_meanframe=Minimas([1:13,17:24,28:36,39:46,48:55,57:end-1]);
...
for i = 1:1:length(max_pks_New)
   if (max_pks_New(i)>1.8e04 && max_pks_New(i)< 2.5e04)
...
for i = 1:1:length(min_pks_New)
   if (min_pks_New(i)<-0.8e04 && min_pks_New(i)> -2.0e04)
...
max_TV= Maximas_New_pks([2:14,16:23,25:33,35:50,52:end-1]);
min_TV= Minimas_pks_New([2:14,18:25,29:37,40:47,50:57,60:end-1]);
...
ratio_VT = Tidal_vol/Body_plyt(7,2);
ratio_ERV = ERV/Body_plyt(7,3);
ratio_IRV = IRV/Body_plyt(7,4);
ratio_FRC = FRC/Body_plyt(7,1);