...
for i = 1:1:length(max_pks)
   if (max_pks(i)>1.0e04 && max_pks(i)< 2.7e04)
...
for i = 1:1:length(min_pks)
   if (min_pks(i)>-1.0e04 && min_pks(i)<-0.3e04)
...
max_meanframe=Maximas([2:13,15:22,24:31]);
min_meanframe=Minimas([1:12,15:22,25:32]);
...
for i = 1:1:length(max_pks_New)
   if (max_pks_New(i)>1.0e04 && max_pks_New(i)< 2.5e04)
...
for i = 1:1:length(min_pks_New)
   if (min_pks_New(i)<-0.30e04 && min_pks_New(i)> -1.0e04)
...
max_TV= Maximas_New_pks([2:13,15:30]);
min_TV= Minimas_pks_New([1:12,15:22,25:32]);
...
ratio_VT = Tidal_vol/Body_plyt(9,2);
ratio_ERV = ERV/Body_plyt(9,3);
ratio_IRV = IRV/Body_plyt(9,4);
ratio_FRC = FRC/Body_plyt(9,1);